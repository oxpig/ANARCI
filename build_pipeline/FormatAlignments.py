...
def format_j_genes(jalignments):
    ...
    if sys.platform == "darwin":
        pr = Popen( [ "muscle_macOS", "-in", ffile, "-gapopen", "-15", "-out", al_filename, ], stdout=PIPE, stderr=PIPE )
    else:
        pr = Popen( [ "muscle", "-in", ffile, "-gapopen", "-15", "-out", al_filename, ], stdout=PIPE, stderr=PIPE )
    o, e = pr.communicate()
